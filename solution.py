...
def naked_twins(values):
    ...
    for unit in [tuple(u) for u in unitlist]:
        ...
        for box in unit:
            # See if val is naked twin -- two numbers and appears
            # twice in unit
            val = values[box]
...
diag_units = [["".join(x) for x in list(zip(rows, cols))],
              ["".join(x) for x in list(zip(rows, reversed(cols)))]]
...
def grid_values(grid):
    ...
    vals = list(grid)
    vals = [val if val != '.' else '123456789' for val in vals]
